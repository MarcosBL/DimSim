 ...
 *
 * $Id: region_more.cpp,v 1.2 2008/09/09 17:38:05 mohsa01 Exp $
...
void region_pack(RegionPtr pregion, int threshold)
{
  ...
  if (sum_overhead) {
    ...
    DDSS_VERBOSE("Region")<<"Joined Rectangles : "<<num_rects<<" -> "<<REGION_NUM_RECTS(pregion)<<", overhead : "<<sum_overhead<<std::endl;
  }
...
static void report_bad_rect_order(void) {
  if (!warned) {
    DDSS_WARN("Region")<<"Bad rectangle order in regions - not packing"<<std::endl;
    warned = 1;
  }
}
