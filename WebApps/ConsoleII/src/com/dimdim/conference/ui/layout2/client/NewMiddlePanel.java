		...
		ClientStateModelListener, ActivePresenterAVListener, PopupListener, UserCallbacks
{
	...
	protected	RosterModelAdapter rosterModelListener = null;
	protected	MainLayout consoleLayout = null;
	protected	ConsoleMiddleLeftPanel leftPanel = null;
	private		boolean	serverConnectionLost = false;
	
	public NewMiddlePanel(MainLayout consoleLayout, UIRosterEntry currentUser)
	{
	...
	public String onWindowClosing()
	{
		...
		else
		{
			...
			if  (this.currentUser.isHost())
			{
				...
				{
					return "If you are sharing a website and do not want to close a meeting, Please click 'Cancel'";
				}
				else
				{
					return	UIConstants.getConstants().presenterDepartureWarning();
				}
			}
			else
			{
				return	UIConstants.getConstants().attendeeDepartureWarning();
			}
		}
	}
...
//		{
			...
			{
				if (closeMeetingOnHostNavigation())
				{
					ClientModel.getClientModel().leaveConference();
				}
			}
...
//		}
	...
	private native boolean closeMeetingOnHostNavigation() /*-{
		try
		{
			return	$wnd.closeMeetingOnHostNavigation();
		}
		catch(e)
		{
			return	false;
...
//		closeBeforeReconnectAttempt();
	...
	{
		this.consoleLayout.getFullPanel().getTopPanel().getSignOutPanel().signout();
	}
...
//		consoleLayout.getFullPanel().getTopPanel().getToolsPanel().onDisplayNameChanged();
	}
