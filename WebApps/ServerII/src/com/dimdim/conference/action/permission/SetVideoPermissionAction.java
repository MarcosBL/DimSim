 ...
 */
public class SetVideoPermissionAction	extends	ConferenceAction
{
	...
	public	SetVideoPermissionAction()
	{
	}
		...
		if (user.isPresenter())
		{
			...
			try
			{
				if (disableAll != null)
				{
					...
					if (b)
					{
						System.out.println("Disabling video permission for all attendees");
						ipm.disableVideoForAll();
					}
				}
				else
				{
					if (enableList != null)
					{
						System.out.println("Enabling video permission for: "+enableList);
						Vector v = this.parseStringToVector(enableList);
						ipm.enableVideoForAllInList(v);
					}
					if (disableList != null)
					{
						System.out.println("Disabling video permission for: "+disableList);
