...
public class Mailer2	{
    ...
    public	Mailer2(ActiveConference conf, IConferenceParticipant sender, String target)	throws	Exception
    {
	...
	protected void initialize(ActiveConference conf, IConferenceParticipant sender,
			String target) throws Exception
	{
		try
		{
			mailerRes = ConferenceConsoleConstants.getDimdimConferenceProperties();
    	}
...
//      Get ssl enabled or not
    ...
    protected InternetAddress getInternetAddress(String emailAddress, String emailPerson,
			ActiveConference conf, IConferenceParticipant sender, String target) throws Exception
    {
    	...
    	throws	Exception
    {
        ...
        try {
           ...
           //     msg.setSubject(emailSubject+" "+subject);
            	//this is tweak to avoid feedback mails getting subject appended with Invitations from ...
        	...
        	msg.setSubject(subject);
            	   if (subject.startsWith("Feedback"))
            	{
            		msg.setSubject(subject+" "+conf.getConferenceInfo().getName());
            	}
            }
            else
            {
            ...
            if(null != sender.getEmail() && sender.getEmail().length() > 0)
            {
            	emailReplyTo[0] = new InternetAddress(sender.getEmail());
            }
            else if(conf.getConferenceInfo().getOrganizerEmail() != null)
            {
            	emailReplyTo[0] = new InternetAddress(conf.getConferenceInfo().getOrganizerEmail());
            }
            else
            {
            	emailReplyTo[0] = new InternetAddress(ConferenceConsoleConstants.getResourceKeyValue("start_meeting_user_email","admin@dimdim.com"));
            }
