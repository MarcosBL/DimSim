...
<html>
	...
	<body>
		...
		<div> user agent string is  = <%=userAgent%></div>
		
		<div> The OS  type = <%=UtilMethods.getOsType(userAgent)%></div>
		
		<div> The Browser  type = <%=UtilMethods.getBrowserType(userAgent)%></div>
		
		<div> The Browser  version = <%=UtilMethods.getBrowserVersion(userAgent)%></div>
	</body>
</html>
