...
#define jpegDstDataLen     cl->jpegDstDataLen
	...
	/* Dimdim specific data and methods -- Bharat Varma */
...
extern rfbClientPtr rfbNewClient();
extern rfbClientPtr rfbReverseConnection(char *host);
...
/* Additional data and methods for Dimdim -- Bharat Varma */
...
dVNCQueue* rfbQueue;
NSDictionary* dimdimProfile;
DimdimMessageClient* msgClient;

extern int sendExact(rfbClientPtr cl, dByteBuffer* buf, NSString* cmd);
extern void sendQueued(char* buf, int len);
extern int flushQueue(rfbClientPtr cl, size_t blockSize);
