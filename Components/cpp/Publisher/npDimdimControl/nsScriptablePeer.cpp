...
static NS_DEFINE_IID(kIDimdimControlIID, NS_IDIMDIMCONTROL_IID);
...
{ 
  ...
  if(aIID.Equals(kIDimdimControlIID)) {
    *aInstancePtr = NS_STATIC_CAST(nsIDimdimControl*, this); 
    AddRef();
    return NS_OK;
  }
  ...
  if(aIID.Equals(kISupportsIID)) {
    *aInstancePtr = NS_STATIC_CAST(nsISupports*,(NS_STATIC_CAST(nsIDimdimControl*, this))); 
    AddRef();
    return NS_OK;
  }
...
//
NS_IMETHODIMP nsScriptablePeer::GetVersion(PRInt32 *_retval)
{
	if (mPlugin)
		mPlugin->getVersion(_retval);
	return NS_OK;
}

NS_IMETHODIMP nsScriptablePeer::PerformAction(const nsAString & args, PRInt32 *_retval)
{
	if (mPlugin)
		mPlugin->performAction(args, _retval);
	return NS_OK;
}


NS_IMETHODIMP nsScriptablePeer::GetProperty(const nsAString & args, nsAString & _retval)
{
	if (mPlugin)
		mPlugin->getProperty(args, _retval);
	return NS_OK;
}

NS_IMETHODIMP nsScriptablePeer::SetProperty(const nsAString & args, PRInt32 *_retval)
{
	if (mPlugin)
		mPlugin->setProperty(args, _retval);
	return NS_OK;
}