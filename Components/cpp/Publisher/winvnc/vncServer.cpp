...
vncServer::vncServer()
{
	...
	m_transactionWaitTime = 100;
	m_bPointerAlgo = FALSE;
	m_bRestrictedColors = FALSE;

	m_iMaxRetries = 2;
	m_iMaxWaitTime = 7;
	m_iStressCount = 20;
	m_bStressInvoked = FALSE;
	m_bCopyRectEnabled = TRUE;
	m_iCompressLevel = 9;
	m_iJPEGLevel = -1;
	m_iBlockSize = 32 * 1024;
...
vncClientId
vncServer::AddClient(CurlSocket *socket, BOOL reverse, BOOL shared)
{
...
{
	vnclog.Print(LL_DIMDIM, VNCLOG("Screencaster 4.0, build 112608\r\n"));
	vncClientList::iterator i;
	omni_mutex_lock l(m_clientsLock);
...
bool vncServer::NotifyDimdim(std::wstring msg)
{
	HANDLE hPipe; 
	BOOL fSuccess; 
