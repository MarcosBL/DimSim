...
public:
	...
	virtual UINT EncodeRect(BYTE *source, CurlSocket *outConn, BYTE *dest, const RECT &rect, int offsetx, int offsety);
	...
	BOOL SendEmptyCursorShape(CurlSocket *outConn);
	BOOL SendCursorShape(CurlSocket *outConn, vncDesktop *desktop);

protected:
	BOOL SetTranslateFunction();

	// Supporting cursor shape updates
	BOOL SendXCursorShape(CurlSocket *outConn, BYTE *mask, int xhot,int yhot,int width,int height);
	BOOL SendRichCursorShape(CurlSocket *outConn, BYTE *mbits, BYTE *cbits, int xhot,int yhot,int width,int height);
