...
inline UINT
vncEncoder::EncodeRect(BYTE *source, CurlSocket *outConn, BYTE *dest, const RECT &rect, int offsetx, int offsety)
{
...
BOOL
vncEncoder::SendEmptyCursorShape(CurlSocket *outConn)
{
	...
	vnclog.Print(LL_DIMDIM, "SENDING EMPTY CURSOR SHAPE\r\n\r\n");

	return outConn->SendQueued((char *)&hdr, sizeof(hdr));
}

BOOL
vncEncoder::SendCursorShape(CurlSocket *outConn, vncDesktop *desktop)
{
...
BOOL
vncEncoder::SendXCursorShape(CurlSocket *outConn, BYTE *mask,
...
BOOL
vncEncoder::SendRichCursorShape(CurlSocket *outConn, BYTE *mbits, BYTE *cbits,
								int xhot, int yhot, int width, int height)
{
	rfbFramebufferUpdateRectHeader hdr;
	hdr.r.x = Swap16IfLE(xhot);
	...
	buff.append(dstbuf,  dstbuf_size);
	buff.append(mbits, mask_size);
	
	if (!outConn->SendQueued((char*)buff.getData(), buff.getLength()))
	{
		delete[] dstbuf;
		return FALSE;
	}
