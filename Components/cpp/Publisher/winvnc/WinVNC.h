...
#define WM_DIMDIM_SCREENCASTER_MSG	WM_USER+8128
...
const char winvncShareHandle[]		= "-sharehandle";
const char winvncSharePrimary[]		= "-shareprimary";

const char winvncAddNewClientURL[]	= "-url";
const char winvncShowHelp[]			= "-help";

const char winvncConnect[]			= "-connect";
const char winvncHost[]				= "-host";
const char winvncPort[]				= "-port";
const char winvncApp[]				= "-app";
const char winvncConfKey[]			= "-confkey";
const char winvncStreamID[]			= "-streamid";

const char winvncStress[]			= "-stress";

// Usage string
const char winvncUsageText[] =
	"dimdimscreencaster [-run] [-kill] \n"
	" [-url http://<host>:<port>/<appName>/<confKey>/<streamID>/]\n"
	" [-connect -host <host> -port <port> -app <appName> -confkey <confkey> -streamid <streamid>]\n"
	" [-shareall] [-sharehandle <window handle>]\n"
	" [-sharewindow  \"title\"] [-help]\n";
