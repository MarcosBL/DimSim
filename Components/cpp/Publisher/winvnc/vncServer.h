...
public:
	...
	virtual vncClientId AddClient(CurlSocket *socket, BOOL reverse, BOOL shared);
	virtual vncClientId AddClient(CurlSocket *socket, BOOL reverse, BOOL shared,
	...
	// Any notification to be issued to the controlling
	// Dimdim Publisher Application
	virtual bool NotifyDimdim(std::wstring msg);
...
/*	    if (autoport && !m_autoportselect)
	...
	*/
	...
	// Dimdim Properties set/get functions
	
	virtual void SetTransactionWaitTime(VInt uVal) { m_transactionWaitTime = uVal; }
	virtual void SetOperationType(UINT uVal) {m_operationType = uVal;}
	virtual void EnablePointerAlgorithm(BOOL bVal) {m_bPointerAlgo = bVal;}
	virtual void EnableRestrictedColors(BOOL bVal) {m_bRestrictedColors = bVal;}
	virtual void SetMaxRetries(VInt iVal) {m_iMaxRetries = iVal;}
	virtual void SetMaxWaitTime(VCard32 iVal) {m_iMaxWaitTime = iVal;}
	virtual void SetStressCount(VInt iVal) {m_iStressCount = iVal;}
	virtual void EnableCopyRect(BOOL bVal) {m_bCopyRectEnabled = bVal;}
	virtual void SetCompressLevel(VInt iVal) {m_iCompressLevel = iVal;}
	virtual void SetJPEGLevel(VInt iVal) {m_iJPEGLevel = iVal;}
	virtual void SetBlockSize(VInt iVal) {m_iBlockSize = iVal;}
	
	VInt GetTransactionWaitTime() { return m_transactionWaitTime; }
	UINT GetOperationType() { return m_operationType; }
	BOOL PointerAlgorithmEnabled() {return m_bPointerAlgo;}
	BOOL RestrictedColorsEnabled() {return m_bRestrictedColors;}
	VInt GetMaxRetries() {return m_iMaxRetries;}
	VCard32 GetMaxWaitTime() {return m_iMaxWaitTime;}
	VInt GetStressCount() { return m_iStressCount; }
	BOOL CopyRectEnabled() { return m_bCopyRectEnabled; }
	VInt GetCompressLevel() { return m_iCompressLevel; }
	VInt GetJPEGLevel() {return m_iJPEGLevel;}
	VInt GetBlockSize() {return m_iBlockSize;}
	...
	void InvokeStress(BOOL bValue) {m_bStressInvoked = bValue;}
	BOOL IsStressInvoked() { return m_bStressInvoked; }
...
// vncVideoDriver::m_fDirectAccessInEffect
	...
	BOOL				m_bPointerAlgo;
	BOOL				m_bRestrictedColors;
	VInt				m_iMaxRetries;
	VCard32				m_iMaxWaitTime;
	VInt				m_iStressCount;
	BOOL				m_bCopyRectEnabled;
	VInt				m_iCompressLevel;
	VInt				m_iJPEGLevel;
	VInt				m_iBlockSize;

	// Stress test related Properties

	BOOL				m_bStressInvoked;

	// 0 - no network / no dump
	// 1 - no network / dump enabled
	// 2 - network enabled / no dump
	// 3 - network enabled / dump enabled
