...
PostToWinVNC(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	SendMessage(hservwnd, message, wParam, lParam);
...
BOOL vncService::PostAddNewClientURL(std::string url)
{
	ATOM urlAtom = GlobalAddAtom(url.c_str());
	// Post to the WinVNC menu window
	if (!PostToWinVNC(MENU_ADD_CLIENT_URL_MSG, (WPARAM)urlAtom, (LPARAM)(url.length() + 1)))
	{
		return FALSE;
	}
