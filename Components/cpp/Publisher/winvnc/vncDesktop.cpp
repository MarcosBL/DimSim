...
vncDesktop::~vncDesktop()
{
	...
	// First re-enable screensaver, in case this is Vista
	EnableVistaScreensaver();
...
//		InvalidateRect(NULL,NULL,TRUE);
	...
	{
		ForceLowProfile("Unable to activate driver");
	}
...
void
vncDesktop::EnableVistaScreensaver()
{
	...
	case VER_PLATFORM_WIN32_NT:
		{
			...
			if (NULL == hDesk)
			{
				// Detected Vista. Without disturbing code for regular Windows NT,
				// here we go...
...
RECT vncDesktop::GetSourceRect()
{
	...
	{
		vnclog.Print(LL_DIMDIM, VNCLOG("Leaving GetSourceRect - ScreenAreaShared\n"));
...
void vncDesktop::ForceLowProfile(std::string reason)
{
	vnclog.Print(LL_STATE, VNCLOG("%s - Internally shifting to low profile...\n"), reason.c_str());
	m_server->EnableRestrictedColors(TRUE);
	m_server->SetJPEGLevel(-1);
	m_server->SetBlockSize(16);
	m_server->EnablePointerAlgorithm(TRUE);
	m_server->NotifyDimdim(L"Warning");
}